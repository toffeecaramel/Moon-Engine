...
/**
 * Made for the retired Horizon Engine by Cobalt.
 * May their engine rest in peace </3
 * https://github.com/CCobaltDev/FNF-Horizon-Engine/blob/rewrite/source/horizon/objects/Alphabet.hx
 */
...
class Alphabet extends FlxSpriteGroup
{
    ...
    private var lines:Array<Array<MoonSprite>> = [];
    ...
    function updateText(?val:String):Void
    {
        ...
        for (i => text in val.replace('\r', '').split('\n'))
        {
            ...
            for (ch in text.split(''))
            {
                ...
                var char = alphabetGroup.recycle(MoonSprite);
                char.frames = Paths.getSparrowAtlas('menus/alphabet');
